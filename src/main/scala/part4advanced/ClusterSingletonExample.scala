...
class PaymentSystem extends Actor with ActorLogging {
  override def receive: Receive = {
    case t: Transaction => log.info(s"Validating transaction $t") // add complex business logic
...
class PaymentSystemNode(port: Int, shouldStartSingleton: Boolean = true) extends App {
  val config = ConfigFactory.parseString(
    ...
    """.stripMargin)
    .withFallback(ConfigFactory.load("part4advanced/ClusterSingletonExample.conf"))
...
object PaymentSYstemClient extends App {
  val config  = ConfigFactory.parseString(
    ...
    """.stripMargin)
    .withFallback(ConfigFactory.load("part4advanced/ClusteringSingletonExample.conf"))
