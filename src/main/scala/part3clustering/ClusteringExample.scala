...
import part3clustering.ClusteringExampleDomain.{ProcessFile, ProcessLine, ProcessLineResult}
  ...
  def handleClusterEvents: Receive = {
    case MemberUp(member) if (member.hasRole("worker")) => {
      ...
      if (pendingRemoval.contains(member.address)) {
        ...
        val workerSelection = context.actorSelection(s"${member.address}/user/work")
    ...
    case UnreachableMember(member) if (member.hasRole("worker")) => {
      ...
      workerOption.foreach { ref => pendingRemoval = pendingRemoval + (member.address -> ref)
      }
    }
...
object SeedNodes extends App {
  ...
  def createNode(port: Int, role: String, props: Props, actorName: String): ActorRef = {
    val config = ConfigFactory
      ...
      """.stripMargin)
      .withFallback(ConfigFactory.load("part3clustering/ClusteringExample.conf"))
  ...
  createNode(2252, "worker", Props[Worker], "worker")
  createNode(2253, "worker", Props[Worker], "worker")
