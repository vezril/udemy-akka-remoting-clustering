...
class ClusterSubscriber extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case MemberUp(member) => {
      log.info(s"Let's say welcome to thhe newest member: ${member.address}")
    }
    case MemberRemoved(member, previousStatus) => {
      log.info(s"Poor ${member.address} it was removed from $previousStatus")
    }
    case UnreachableMember(member) => {
      log.info(s"Uh oh, member ${member.address} is unreachble.")
    }
    case m: MemberEvent => {
      log.info(s"Another member info: $m")
    }
  }
}
  ...
  def startCluster(ports: List[Int]): Unit =
    ports.foreach { port =>
      val config = ConfigFactory
        ...
        """.stripMargin)
        .withFallback(ConfigFactory.load("part3clustering/ClusteringBasics.conf"))
...
object ClusteringBasicsManualRegistration extends App {
  val system = ActorSystem("RTJVMCluster",
                           ConfigFactory.load("part3clustering/ClusteringBasics.conf").getConfig("manualRegistration"))
  ...
  def joinExistingNode = cluster.join(Address("akka", "RTJVMCluster", "localhost", 2551))
