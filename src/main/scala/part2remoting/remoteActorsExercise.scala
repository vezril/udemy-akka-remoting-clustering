...
class WordCountMaster extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case Initialize(nWorkers) => {
      log.info("Mater intializing...")
      val workerSelections =
        (1 to nWorkers).map(id => context.actorSelection(s"akka://WorkerSystem@localhost:2552/user/wordCountWorker$id"))
    ...
    def online(workers: List[ActorRef], remainingTasks: Int, totalCount: Int): Receive = {
      case text: String => {
        ...
        // send senteces to worker in turn
...
object MasterApp extends App {
  ...
  val config = ConfigFactory.parseString("""
    ...
    """.stripMargin).withFallback(ConfigFactory.load("part2remoting/remoteActorsExercise.conf"))
