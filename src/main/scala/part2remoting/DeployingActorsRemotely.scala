...
class DeployingActorsRemotely_LocalApp extends App {

  val system = ActorSystem("LocalActorSystem",
                           ConfigFactory.load("part2remoting/deployingActorsRemotely.conf").getConfig("localApp"))
  ...
  println(simpleActor) // expected: akka://RemoteActorSystem@localhost:2552/user/remoteActor
...
class DeployingActorsRemotely_RemoteApp extends App {

  val system = ActorSystem("RemoteActorSystem",
                           ConfigFactory.load("part2remoting/deployingActorsRemotely.conf").getConfig("remoteApp"))
}
